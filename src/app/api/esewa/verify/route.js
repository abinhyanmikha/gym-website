...
export async function POST(req) {
  try {
    ...
    console.log("Received verification request:", { amount, refId, userId, subscriptionName, subscriptionId });
    ...
    if (!subscriptionName) {
      ...
      return NextResponse.json({ error: "Missing subscriptionName" }, { status: 400 });
    }
    if (!subscriptionId) {
    ...
    try {
      ...
      console.log("Payment updated:", updatedPayment ? updatedPayment._id : "Not found");
    } catch (paymentError) {
    ...
    try {
      ...
      if (subscriptionName.toLowerCase().includes('monthly') || subscriptionName.toLowerCase().includes('basic')) {
        durationInDays = 30;
      } else if (subscriptionName.toLowerCase().includes('quarterly') || subscriptionName.toLowerCase().includes('standard')) {
        durationInDays = 90;
      } else if (subscriptionName.toLowerCase().includes('yearly') || subscriptionName.toLowerCase().includes('premium')) {
        durationInDays = 365;
      }
      
      console.log(`Subscription duration: ${durationInDays} days for ${subscriptionName}`);
